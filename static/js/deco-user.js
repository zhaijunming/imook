...
function sendCodeChangeEmail($btn){
    ...
    $.ajax({
        ...
        success: function(callback){
            if(callback.email){
                Dml.fun.showValidateError($('#jsChangeEmail'), callback.email);
...
var verify = verifyDialogSubmit(
    ...
    $.ajax({
        ...
        beforeSend:function(XMLHttpRequest){
            $btn.val("发送中...");
            $btn.attr('disabled',true);
            $("#jsChangeEmailTips").html("验证中...").show(5);
        },
...
$(function(){
    ...
    $('#jsResetPwdBtn').click(function(){
        $.ajax({
            ...
            async: true,
            success: function(callback) {
                if(callback.password1){
                    Dml.fun.showValidateError($("#pwd"), callback.password1);
                }else if(callback.password2){
                    Dml.fun.showValidateError($("#repwd"), callback.password2);
                }else if(callback.status == "success"){
                    ...
                    Dml.fun.showValidateError($("#pwd"), callback.msg);
                    Dml.fun.showValidateError($("#repwd"), callback.msg);
                }
            }
        });
    });
